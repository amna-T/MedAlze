...
import { XRayRecord, Appointment } from '@/types/database'; // Import Appointment
...
const PatientDashboard = () => {
    ...
    const fetchPatientData = async () => {
      ...
      try {
        ...
        if (!patientIdForQuery) {
          console.warn("Patient ID not found for current user. Cannot fetch X-rays or appointments.");
          setLoading(false);
          return;
        }
        ...
        const patientXrays: XRayRecord[] = xraysSnapshot.docs.map(doc => ({ ...doc.data() as XRayRecord, id: doc.id }));
        ...
        const upcomingAppointments = patientAppointments.filter(app => 
          app.status === 'scheduled' && new Date(app.preferredDate) >= now
        ).length;

        const pendingRequests = patientAppointments.filter(app => app.status === 'pending').length;
  ...
  const dashboardStats = [
    ...
    { title: 'Upcoming Appointments', value: stats.upcomingAppointments.toString(), icon: Calendar, change: 'Scheduled' },
    { title: 'Latest Result', value: stats.latestResult, icon: Activity, change: 'Recently analyzed' },
    { title: 'Pending Requests', value: stats.pendingRequests.toString(), icon: Clock, change: 'Awaiting scheduling' },
  ];
            ...
            <div className="space-y-3">
              <Link to="/upload-request" className="block">
                <button className="w-full p-4 rounded-lg bg-primary/10 hover:bg-primary/20 transition-colors text-left">
                  <p className="font-medium">Request New Appointment</p>
                  <p className="text-sm text-muted-foreground">Schedule a new scan or consultation</p>
                </button>
              </Link>
